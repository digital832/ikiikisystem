...
const INSURERS_HEADER = ["ID", "保険者番号", "登録日時", "保険者名"];
...
function rowToConsent(row) {
  return {
    ...
    examDate: row[11] || "",
    onsetDate: row[12] || "",
    institutionName: row[13] || "",
    institutionAddress: row[14] || "",
    noticeConfirmed: row[15] === "1",
    diseaseName: row[16] || "",
    muscleParesisAreas: splitList(row[17]),
    jointContractureAreas: splitList(row[18]),
    jointContractureOther: row[19] || "",
    numbnessAreas: splitList(row[20]),
    painAreas: splitList(row[21]),
    motorDysfunctionAreas: splitList(row[22]),
    symptomOther: row[23] || "",
    edemaDisuse: row[24] === "1",
    massageAreas: splitList(row[25]),
    manipulationAreas: splitList(row[26]),
    visitNeed: row[27] || "",
    visitReason: row[28] || "",
    careLevel: row[29] || "",
    shinkyuDiseases: splitList(row[30]),
    shinkyuDiseaseOther: row[31] || "",
    notes: row[32] || "",
  };
}
...
async function addConsent(consent) {
  ...
  const row = [
    ...
    consent.institutionAddress || "",
    consent.noticeConfirmed ? "1" : "",
    consent.diseaseName || "",
    (consent.muscleParesisAreas || []).join("・"),
    (consent.jointContractureAreas || []).join("・"),
    consent.jointContractureOther || "",
    (consent.numbnessAreas || []).join("・"),
    (consent.painAreas || []).join("・"),
    (consent.motorDysfunctionAreas || []).join("・"),
    consent.symptomOther || "",
    consent.edemaDisuse ? "1" : "",
    (consent.massageAreas || []).join("・"),
    (consent.manipulationAreas || []).join("・"),
    consent.visitNeed || "",
    consent.visitReason || "",
    consent.careLevel || "",
    (consent.shinkyuDiseases || []).join("・"),
    consent.shinkyuDiseaseOther || "",
    consent.notes || "",
  ];
  await sheets.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    range: `${CONSENTS_SHEET}!A:AG`,
    valueInputOption: "RAW",
    requestBody: { values: [row] },
  });
...
async function getInsurers() {
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID,
    range: `${INSURERS_SHEET}!A2:D`,
  });
  const rows = res.data.values || [];
  return rows
    .filter((row) => row[0])
    .map((row) => ({ id: row[0], number: row[1] || "", name: row[3] || "" }));
}

async function addInsurer(insurer) {
  const id = "in" + Date.now();
  const name = (insurer && insurer.name) || "";
  const number = (insurer && insurer.number) || "";
  await sheets.spreadsheets.values.append({
    spreadsheetId: SPREADSHEET_ID,
    range: `${INSURERS_SHEET}!A:D`,
    valueInputOption: "RAW",
    requestBody: { values: [[id, number, new Date().toISOString(), name]] },
  });
