...
async function ensureSheets() {
  ...
  const toCreate = [USERS_SHEET, FACILITIES_SHEET, STAFF_SHEET, CLINICS_SHEET, RECORDS_SHEET, DOCTORS_SHEET, CONSENTS_SHEET, EMPLOYEES_SHEET, SETTINGS_SHEET].filter((name) => !existingTitles.includes(name));
...
async function updateUser(id, user) {
  const found = await findRowById(USERS_SHEET, "AC", id);
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: SPREADSHEET_ID,
    range: `${USERS_SHEET}!A${found.rowNumber}:AC${found.rowNumber}`,
    valueInputOption: "RAW",
    requestBody: { values: [row] },
  });
  return rowToUser(row);
...
async function deleteUser(id) {
  return deleteRowById(USERS_SHEET, "AC", id);
}
